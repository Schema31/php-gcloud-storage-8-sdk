...
if ($argc < 4) {
    die("\n\nAttenzione!\n\nNon hai specificato abbastanza parametri: \n" . $argv[0] . " <repositoryName> <Authentication> <fileKey> [<fileVersion>] [<expiresAt>] [<whitelist>]\n\n");
}
...
$expiresAt = (array_key_exists(5, $argv) ? $argv[5] : NULL); // ATTRIBUTO FACOLTATIVO
$whitelist = (array_key_exists(6, $argv) ? $argv[6] : NULL); // ATTRIBUTO FACOLTATIVO

$Storage = new gCloud_Storage();
$Storage->repositoryName = $repositoryName;
$Storage->Authentication = $Authentication;

$response = $Storage->getShortUrl($fileKey, $fileVersion, $expiresAt, $whitelist);
